...
object PlayJsonUtils {
  ...
  implicit class JsObjectImplicits(json: JsObject) {
    def parseDoubleOption(fieldName: String, projectionOption: Boolean): Option[Double] =

      if (projectionOption) Some(PlayJsonUtils.parseDoubleOption(json, fieldName).getOrElse(0)) else None

    def parseObjectArray(fieldName: String): Seq[JsObject] =
      PlayJsonUtils.parseObjectArray(json, fieldName)
  }

  def parseDoubleOption(json: JsObject, fieldName: String): Option[Double] =
    parseOption(json, fieldName).map(_.as[Double])

  def parseObjectArray(json: JsObject, fieldName: String): Seq[JsObject] =
    parseArray(json, fieldName).map(_.as[JsObject])

  def parseArray(json: JsObject, fieldName: String): Seq[JsValue] =
    (json \ fieldName).as[JsArray].value.toVector

  def parseOption(json: JsObject, fieldName: String): Option[JsValue] =
