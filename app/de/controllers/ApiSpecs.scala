...
import com.iheart.playSwagger.MapVariablesTransformer
...
class ApiSpecs @Inject() extends Controller {
  ...
  val transformers = MapVariablesTransformer(Map("GeneInfoObj" -> "de.model.output.GeneInfo"))
  private lazy val generator = SwaggerSpecGenerator(Seq(transformers),domainPackage)
